...
public class MarkTaskAsDoneCommandHandler : IRequestHandler<MarkTaskAsDoneCommand, TaskItem>
{
    ...
    {
        TaskItem taskItem = await _taskItemRepository.GetOneTask(request.Id) ?? throw new IdGivenTaskNotFoundException();
        if (taskItem.IsComplete) throw new TaskAlreadyCompletedException();
